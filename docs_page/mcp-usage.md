...
| `text_table` | Text table (delivery class C/S DB table with one LANG key field, e.g. `T005T`) |

> **`text_table` (translatable text tables).** A database table whose **delivery class is C or S** with **exactly one** language key field (LANG, e.g. `SPRAS`) is a text table; its non-key character columns (e.g. `LANDX` on `T005T`) are the text attributes. It takes two extra params beyond `object_name`/`language`: **`language_key_field_name`** (the LANG key field, e.g. `SPRAS`) and **`master_key_fields`** (`[{ name, value }]` fixing **all** master keys — every key except the language field — to pin one record, e.g. `[{ "name": "LAND1", "value": "DE" }]`). On `TranslateSetTexts`, each `texts` entry's `attribute` is a **text column name** (e.g. `LANDX`), not a UI label. On `TranslateGetTexts`, the columns are enumerated automatically and the filled ones returned (`language` may be omitted → connection language). A table with no language field (e.g. delivery class W) is rejected by XCO. See [`text-table.md`](./text-table.md) for the full request/response shapes.

> **`cds_entity` (one named entity + its own DDLX).** A CDS entity's labels are split across two physical objects — the view itself (`data_definition`) and its metadata extension/DDLX (`metadata_extension`). `cds_entity` treats those two as one: `TranslateGetTexts` fans out to **both** and concatenates the result (the DDLX labels come back without a separate `metadata_extension` call), tagging every CDS row with an **`owner`** of `"data_definition"` or `"metadata_extension"`. `TranslateSetTexts` reads that `owner` back and routes each row to the matching object, so each is locked/transported **once**. Rows are **not** deduplicated across owners. `data_definition` and `metadata_extension` stay available to target one physical object explicitly (single-owner, unmerged).
>
> **Scope — one entity, not a whole RAP stack.** `cds_entity` covers the **named entity and its own DDLX only**. It does **not** reach the underlying/parent views of an `as projection on` chain — e.g. the interface view `ZI_…` behind a projection `ZC_…`, which carries its **own** `@EndUserText.label`. Translating a stack means **one `cds_entity` pass per distinct entity** (one view = one call): the `ZC_` projection, the `ZI_` interface view, and any further underlying views each need their own call. `cds_entity` is valid **with or without** a DDLX — on an entity with no metadata extension the `data_definition` is still written and the DDLX sub-call just finds nothing (the call stays valid but may report a partial status); for such a single-object view, calling `data_definition` directly avoids the empty DDLX sub-call.
...
> **Tip — CDS entities:** start from `target_type: "cds_entity"` in steps 2–4. For **one** entity, a single read returns the view *and* its own DDLX together (each row carrying `owner`), and one `TranslateSetTexts` writes both back, each object locked/transported once — no need to drive `data_definition` and `metadata_extension` separately. But `cds_entity` covers **one named entity + its DDLX only**: on a RAP stack (`ZC_` projection `as projection on` a `ZI_` interface view, …) run **one `cds_entity` pass per entity** — the projection does **not** carry the interface view's `@EndUserText.label`, so the `ZI_` view needs its own call.
