...
| `capabilities` | `{}` | `{ list_texts: [target_type…], set_translation: [target_type…] }` — an **allow-list**: each action lists the object types this stack can translate. To remove a possibility, delete the type from the relevant list in `handle_capabilities`. Public cloud / BTP ABAP Environment and on-premise / private cloud support **different** object types (e.g. `text_pool` is writable but not listable on the cloud stack, so it appears under `set_translation` but not `list_texts`). The set can also differ by **system version**. The MCP server probes this and rejects a `target_type` not on the list up-front; older handlers without the action stay permissive and rely on the `CLOUD_UNSUPPORTED` error. **You do NOT list `cds_entity`** here — it is a LISA-only virtual target (the MCP fans it out to `data_definition` + `metadata_extension`, so the handler never receives it). LISA advertises it automatically for an action whenever **both** `data_definition` and `metadata_extension` are listed for that action. |
