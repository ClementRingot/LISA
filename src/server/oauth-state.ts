 ...
 *
 * The fix: insert this server into the return path. Send XSUAA a `state` we
 * control that is immune to the `+` bug, then re-emit the client's ORIGINAL
 * `state` correctly when redirecting back to the client.
 *
 ...
 * The client's real `state` rides INSIDE the opaque base64url payload, so it
 * survives the XSUAA round-trip untouched.
 *
 * ── Why stateless (vs an in-memory map) ───────────────────────────────
 * Mirrors the StatelessDcrClientStore design: the token carries its own
 * payload + HMAC signature, so any instance with the same signing key can
 * validate it. No in-memory map → survives `cf restart`, cell moves, and
 * horizontal scale-out. The signing key is derived (HKDF-style) from the same
 * secret the DCR store uses, with a distinct domain-separation label so the
 * two key spaces never overlap.
 */
...
interface StatePayload {
  ...
  s?: string;
  /** The OAuth client's ORIGINAL `redirect_uri` — where we send the user
   *  after XSUAA returns to our callback. */
  r: string;
   ...
   *  `redirect_uri` on a victim's signed state. */
  cid: string;
