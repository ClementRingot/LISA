 ...
 * or local-disk store, every CF push / restart wipes the server-side registry
 * — the cached `client_id` then fails with `invalid_client` and the user has
 * to clear their MCP client's OAuth cache to recover.
 *
 * This store eliminates the storage problem entirely. Each `client_id` is a
 * self-validating token: it carries the registration payload (redirect_uris,
 * grant_types, …) plus an HMAC-SHA256 signature derived from a server-held
 * key. `getClient` re-derives the payload by verifying the signature; no
 * persistence is needed. Any process with the same signing key can validate
 * any client_id ever issued.
 *
 * Tradeoffs vs a persisted in-memory store:
 ...
 *
 * Default TTL is 30 days. Setting `ttlSeconds` to `0` disables expiration.
 */
...
/**
 * Truncated HMAC-SHA256 length in bytes. 16 bytes = 128 bits, well above the
 * practical forgery threshold for opaque IDs.
 */
const SIG_BYTES = 16;

/** Default lifetime of a DCR registration (30 days). `0` disables expiration. */
 ...
 *
 * The callback proxy (see `oauth-state.ts`) sends XSUAA our OWN `/oauth/callback`
 * as the redirect_uri and carries the client's real redirect_uri inside the
 * signed state. XSUAA therefore no longer validates the client's redirect_uri —
 * we do. Without an allowlist, `ensureRedirectUri` would auto-trust ANY
 * redirect_uri supplied at `/authorize` for the shared default client, letting
 * an attacker steer a victim's authorization code to their own URI.
 *
 * Vendored (not read from xs-security.json) because xs-security.json is consumed
 * by XSUAA at service-creation time and is not shipped with the running app.
 * Keep the two in sync when adding a client.
 *
 ...
 * so it gives the same answer on every instance.
 *
 ...
 * the glob decision MUST agree with how the URL actually parses. We reject
 * anything that doesn't parse, and reject any userinfo (`user[:pass]@`) — that
 * `@` is the only construct that can relocate the authority past a same-segment
 * wildcard (`http://localhost:x@evil.com/cb` matches the `localhost:[^/]*` port
 * glob yet `new URL(...).host === 'evil.com'`).
 */
...
/**
 * Compact JSON shape stored inside the signed `client_id`. Keys are short to
 * keep the resulting URL-safe `client_id` small (it is sent in `/authorize`
 * query strings and `client_id` form fields).
 */
...
export interface StatelessDcrClientStoreOptions {
  /** How long an issued client_id remains valid, in seconds. Default 30 days.
   *  Set to `0` (or any non-positive value) to disable expiration. */
  ttlSeconds?: number;
...
function buildXsuaaDefaultClient(clientId: string, clientSecret: string): OAuthClientInformationFull {
  return {
    ...
    client_name: 'SAP Translator XSUAA Default Client',
  };
}
...
export class StatelessDcrClientStore implements OAuthRegisteredClientsStore {
  ...
  constructor(
    ...
    // Defense-in-depth: warn (don't throw) on weak signing secrets. 128 bits /
    ...
    // used directly to sign client_ids. The KDF_LABEL doubles as a domain separator.
  ...
  async getClient(clientId: string): Promise<OAuthClientInformationFull | undefined> {
    ...
    if (this.ttlSeconds > 0) {
      ...
      if (ageSec > this.ttlSeconds) {
        ...
        getLogger().debug('OAuth client expired (TTL)', { clientId, ageSec, ttlSeconds: this.ttlSeconds });
        return undefined;
      }
    }
  ...
  async registerClient(
    ...
    getLogger().debug('OAuth client registered (stateless)', {
      clientId,
    ...
    // `client_secret` is issued. Absolute expiry in epoch seconds, or exactly 0
    // if the secret never expires (ttlSeconds <= 0).
  ...
  /**
   * Called before redirect_uri validation on `/authorize`. For the
   * pre-registered XSUAA client we mutate the in-memory list so the SDK's
   * exact-match check passes (replayed on every `/authorize`, so it needn't
   * persist). SECURITY: a candidate URI is registered ONLY if it matches
   * `XSUAA_REDIRECT_URI_PATTERNS`. The callback proxy removed XSUAA from the
   * client-redirect path, so an un-gated add here would let an attacker register
   * an arbitrary redirect_uri and have the SDK accept it. For DCR (`sapt-…`)
   * clients we are stateless by design: there's nothing to mutate.
   */
  ensureRedirectUri(clientId: string, uri: string): void {
    ...
    if (!matchesXsuaaRedirectPattern(uri)) {
      getLogger().warn('Dynamic redirect_uri rejected for XSUAA default client (not in allowlist)', { clientId, uri });
      getLogger().emitAudit({
        level: 'warn',
    ...
    getLogger().debug('Dynamic redirect_uri registered for XSUAA client', { clientId, uri });
    getLogger().emitAudit({
      level: 'info',
   ...
   *    allowlist (`matchesXsuaaRedirectPattern`). Consults the static allowlist,
   *    NOT the mutable in-memory list, so the verdict is stateless and identical
   *    on every instance.
   *  - DCR (`sapt-…`) client → must be one of the redirect_uris baked immutably
  ...
  /**
   * The client_secret is derived deterministically from the client_id, so any
   * instance with the same signing key can validate it. This is the core reason
   * DCR survives container restarts and scales out horizontally with no shared state.
   */
  ...
  private emitLookupFailed(
    ...
    getLogger().debug('OAuth client lookup failed', { clientId, reason });
    getLogger().emitAudit({
